/**
 * Products bar chart with dark/light mode support
 * Shows top products by revenue or sales volume
 */
...
export function ProductsBarChart({
  ...
  // Process data with truncated names
  const processedData = data.slice(0, 8).map((item) => ({
    ...item,
    displayName: truncateProductName(
      item.name,
      layout === "horizontal" ? 15 : 25
    ),
  }));
  ...
  const CustomTooltip = ({ active, payload, label }: any) => {
    if (active && payload && payload.length) {
      const originalItem = data.find(
        (item) =>
          truncateProductName(item.name, layout === "horizontal" ? 15 : 25) ===
          label
      );
